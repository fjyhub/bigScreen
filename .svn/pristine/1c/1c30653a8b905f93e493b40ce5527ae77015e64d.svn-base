<!--  -->
<template>
  <div>
    <div class="card-title">
      <span><img src="../../../assets/imgs/1028.png" alt="" /></span>
      高效能
    </div>
    <div>
      <div class="fz34 fw600 ta-center chart-title" style="color: #00ECF2">
        经济负载设备占比
        <img
          src="../../../assets/imgs/arrow.png"
          alt=""
          style="height: 28px;
    padding: 4px 20px;"
        />
        <h4
          class="sub-title color00c0ff"
          style="font-size: 34px;margin-top: -12px;padding-right: 40px"
        >
          {{ provinceGetData.economicLoadEquipRate }}%
        </h4>
      </div>
    </div>
    <div class="between" style="padding: 20px 50px;width: 660px">
      <div class="left-round direction-column align-center">
        <div class="relative">
          <div id="left-round" style="height: 110px;width: 110px"></div>
          <x :width="140" class="absolute" style="top: -14px;left: -14px"></x>
        </div>
        <div class="fz28 mt20">经济负载主变占比</div>
      </div>
      <div class="right-round direction-column align-center">
        <div class="relative">
          <div id="right-round" style="height: 110px;width: 110px"></div>
          <x :width="140" class="absolute" style="top: -14px;left: -14px"></x>
        </div>
        <div class="fz28 mt20">经济负载线路占比</div>
      </div>
    </div>
    <div class="fz34 fw600 ta-center chart-title" style="color: #00ECF2">
      综合线损率
      <h4 class="sub-title color00c0ff">
        单位：%
      </h4>
    </div>

    <div id="bar1" style="width: 660px;height: 255px" ref="bar1"></div>
    <div class="fz34 fw600 ta-center chart-title" style="color: #00ECF2">
      万元GDP能耗
      <span class="sub-title color00c0ff">
        单位：吨标煤
      </span>
    </div>

    <div id="bar2" style="width: 660px;height: 255px;"></div>
  </div>
</template>

<script>
import * as Echarts from 'echarts'
export default {
  props: {
    provinceGetData: {
      require: true,
      type: Object
    }
  },
  data() {
    return {
      zhxslChart: null,
      gdpChart: null,
      leftRoundChart: null,
      rightRoundChart: null
    }
  },

  components: {},

  computed: {},

  mounted() {
    // this.zhxslChart = Echarts.init(document.getElementById('bar1'), 'dark')
    this.zhxslChart = Echarts.init(this.$refs.bar1, 'dark')

    this.loadZhxslChart()
    this.gdpChart = Echarts.init(document.getElementById('bar2'), 'dark')
    this.loadGdpChart()
    this.leftRoundChart = Echarts.init(document.getElementById('left-round'), 'dark')
    this.loadLeftChart()
    this.rightRoundChart = Echarts.init(document.getElementById('right-round'), 'dark')
    this.loadRightChart()
  },

  methods: {
    loadRightChart() {
      const handred = 100
      let point = this.provinceGetData.economicLineEquipRate

      const option = {
        title: {
          text: point + '%',
          x: 'center',
          y: 'center',
          textStyle: {
            fontWeight: 'normal',
            color: '#29EEF3',
            fontSize: '30'
          }
        },
        tooltip: {
          formatter: function(params) {
            return params.name + '：' + params.percent + ' %'
          }
        },
        series: [
          {
            name: 'circle',
            type: 'pie',
            clockWise: true,
            radius: ['80%', '100%'],
            itemStyle: {
              normal: {
                label: {
                  show: false
                },
                labelLine: {
                  show: false
                }
              }
            },
            hoverAnimation: false,
            data: [
              {
                value: point,
                name: '占比',
                itemStyle: {
                  normal: {
                    color: {
                      // 颜色渐变
                      colorStops: [
                        {
                          offset: 0,
                          color: '#64fefe' // 0% 处的颜色
                        },
                        {
                          offset: 1,
                          color: '#64fefe' // 100% 处的颜色1
                        }
                      ]
                    },
                    label: {
                      show: false
                    },
                    labelLine: {
                      show: false
                    }
                  }
                }
              },
              {
                name: '剩余',
                value: handred - point,
                itemStyle: {
                  normal: {
                    color: '#42a7b3'
                  }
                }
              }
            ]
          }
        ]
      }
      this.rightRoundChart.setOption(option)
    },
    loadLeftChart() {
      const handred = 100
      let point = this.provinceGetData.economicMainTransEquipRate

      const option = {
        title: {
          text: point + '%',
          x: 'center',
          y: 'center',
          textStyle: {
            fontWeight: 'normal',
            color: '#29EEF3',
            fontSize: '30'
          }
        },
        tooltip: {
          formatter: function(params) {
            return params.name + '：' + params.percent + ' %'
          }
        },
        series: [
          {
            name: 'circle',
            type: 'pie',
            clockWise: true,
            radius: ['80%', '100%'],
            itemStyle: {
              normal: {
                label: {
                  show: false
                },
                labelLine: {
                  show: false
                }
              }
            },
            hoverAnimation: false,
            data: [
              {
                value: point,
                name: '占比',
                itemStyle: {
                  normal: {
                    color: {
                      // 颜色渐变
                      colorStops: [
                        {
                          offset: 0,
                          color: '#64fefe' // 0% 处的颜色
                        },
                        {
                          offset: 1,
                          color: '#64fefe' // 100% 处的颜色1
                        }
                      ]
                    },
                    label: {
                      show: false
                    },
                    labelLine: {
                      show: false
                    }
                  }
                }
              },
              {
                name: '剩余',
                value: handred - point,
                itemStyle: {
                  normal: {
                    color: '#42a7b3'
                  }
                }
              }
            ]
          }
        ]
      }
      this.leftRoundChart.setOption(option)
    },
    loadZhxslChart() {
      let img1 =
        'image://data:image/png;base64,'

      let img2 =
        'image://data:image/png;base64,'
      const option = {
        xAxis: {
          type: 'category',
          splitLine: {
            show: false // 是否显示坐标得横纵线
          },
          axisLabel: {
            fontSize: 30,
            // fontWeight: 600,
            color: function(param, index) {
              let color = ['#FFF100', '#00FFFF', '#00FFFF']
              return color[index]
            }
          },
          data: ['浙江', '韩国', '德国']
        },
        yAxis: {
          type: 'value',
          axisLabel: {
            show: true,
            // formatter: '{value}{b|%}{b|icon}',
            formatter: '{value} {icon| }',
            color: '#fff',
            fontSize: 20,
            rich: {
              icon: {
                width: 10,
                height: 10,
                lineHeight: 15,
                align: 'right',
                verticalAlign: 'center',
                backgroundColor: {
                  image: require('../../../assets/imgs/xiaoyuanquan.png')
                }
              }
            }
          }
        },
        grid: {
          show: false
        },
        series: [
          {
            data: [
              {
                value: 3.7,
                itemStyle: {
                  color: new Echarts.graphic.LinearGradient(0, 0, 0, 1, [
                    { offset: 1, color: '#DC7302' },
                    { offset: 0.5, color: '#F2DB1C' },
                    { offset: 0, color: '#F9F35F' }
                  ])
                }
              },
              3.3,
              3.4
            ],
            type: 'bar',
            z: 2,
            barWidth: 54,
            itemStyle: {
              normal: {
                label: {
                  formatter: data => {
                    return data?.value
                  },
                  show: true, //开启显示
                  position: 'top', //在上方显示
                  offset: [0, -10],
                  //数值样式
                  color: '#fff',
                  fontSize: 20
                },
                // barBorderRadius: [10, 10, 0, 0],
                color: new Echarts.graphic.LinearGradient(0, 0, 0, 1, [
                  { offset: 1, color: '#06A06F' },
                  { offset: 0, color: '#3EDAE8' }
                ])
              }
            }
          },
          {
            z: 1,
            type: 'pictorialBar',
            symbolPosition: 'end',
            // data: [3.7, 2.7, 1.3],

            symbolOffset: [0, -40],
            symbolSize: [60, 30],
            data: [
              {
                value: 3.7,
                symbol: img1
              },
              {
                value: 3.3,
                symbol: img2
              },
              {
                value: 3.4,
                symbol: img2
              }
            ]
          }
        ]
      }
      this.zhxslChart.setOption(option)
    },
    loadGdpChart() {
      let img1 =
        'image://data:image/png;base64,'

      let img2 =
        'image://data:image/png;base64,'

      const option = {
        xAxis: {
          type: 'category',
          splitLine: {
            show: false // 是否显示坐标得横纵线
          },
          axisLabel: {
            fontSize: 30,
            // fontWeight: 600,
            color: function(param, index) {
              let color = ['#FFF100', '#00FFFF', '#00FFFF']
              return color[index]
            }
          },
          data: ['浙江', '韩国', '德国']
        },
        yAxis: {
          type: 'value',
          axisLabel: {
            show: true,
            formatter: '{value} {icon| }',
            color: '#b7fdff',
            fontSize: 18,
            rich: {
              icon: {
                width: 10,
                height: 10,
                lineHeight: 15,
                align: 'left',
                verticalAlign: 'top',
                backgroundColor: {
                  image: require('../../../assets/imgs/xiaoyuanquan.png')
                }
              }
            }
          }
        },
        grid: {
          show: false
        },
        series: [
          {
            data: [
              {
                value: 0.39,
                itemStyle: {
                  color: new Echarts.graphic.LinearGradient(0, 0, 0, 1, [
                    { offset: 1, color: '#DC7302' },
                    { offset: 0.5, color: '#F2DB1C' },
                    { offset: 0, color: '#F9F35F' }
                  ])
                }
              },
              0.41,
              0.17
            ],
            type: 'bar',
            z: 2,
            barWidth: 54,
            itemStyle: {
              normal: {
                label: {
                  formatter: data => {
                    return data?.value
                  },
                  show: true, //开启显示
                  position: 'top', //在上方显示
                  offset: [0, -10],
                  //数值样式
                  color: '#fff',
                  fontSize: 20
                },
                // barBorderRadius: [10, 10, 0, 0],
                color: new Echarts.graphic.LinearGradient(0, 0, 0, 1, [
                  { offset: 1, color: '#06A06F' },
                  { offset: 0, color: '#3EDAE8' }
                ])
              }
            }
          },
          {
            z: 1,
            type: 'pictorialBar',
            symbolPosition: 'end',
            symbolOffset: [0, -40],
            symbolSize: [60, 30],
            //  data: [0.37, 0.24, 0.26],
            data: [
              {
                value: 0.39,
                symbol: img1
              },
              {
                value: 0.41,
                symbol: img2
              },
              {
                value: 0.17,
                symbol: img2
              }
            ]
          }
        ]
      }
      this.gdpChart.setOption(option)
    }
  }
}
</script>
<style lang="less" scoped>
.card-title {
  font-size: 34px;
  font-weight: 600;
  line-height: 60px;
  letter-spacing: 8px;
  background-image: -webkit-gradient(
    linear,
    left top,
    left bottom,
    color-stop(0, #00c0ff),
    color-stop(1, #b7fdff)
  );
  color: transparent;
  background-clip: text;
  span {
    letter-spacing: 1px !important;
  }
}
.chart-title {
  position: relative;
  text-align: left;
  text-indent: 1em;
  background-image: -webkit-gradient(
    linear,
    left top,
    left bottom,
    color-stop(0, #00c0ff),
    color-stop(1, #b7fdff)
  );
  color: transparent !important;
  -webkit-background-clip: text;
  -moz-background-clip: text;
  -ms-background-clip: text;
  &::before {
    content: '';
    position: absolute;
    left: 12px;
    top: 3px;
    width: 5px;
    height: 30px;
    background: #00c0ff;
  }
}
.sub-title {
  float: right;
  vertical-align: baseline;
  padding-right: 100px;
  padding-top: 10px;
  font-size: 24px;
  color: #fff100;
}
.color00c0ff {
  color: #00c0ff !important;
}
</style>
